...
export default class ChannelStore {
	...
	public async unblockUserFromChannel({
		channelId,
		userId
	}: {
		channelId: string
	...
	public async updateChannelProperties({
		channelId,
		updatedProperties
	}: {
		channelId: string
	...
	public async addChannelNotificationSubscriber({
		channelId,
		userId
	}: {
		channelId: string
	...
	public async removeChannelNotificationSubscriber({
		channelId,
		userId
	}: {
		channelId: string
	...
	public async getChannels({ isRefresh = false }: { isRefresh?: boolean } = {}) {
		...
		return await fetch(
			`${env.PUBLIC_API_URL}/channels?searchQuery=${this.searchQuery}&category=${this.filterTechList
				.map((item: any) => item.item_text)
				.join()}&skip=${this.skip}&limit=${this.limit}`,
			{
				method: 'GET'
			}
		).then(async (response) => {
