...
{:then value}
	{#if value?.length > 0}
		<div class={'relative p-1 transition-opacity ease-in duration-100 ' + swiperClass}>
			<div class="btn btn-neutral text-white p-2 btn-prev absolute top-2/4 left-1 z-10 ml-3">
				<IconDrawerLeft />
			</div>
			...
			<div class="btn btn-neutral text-white z-10 p-2 btn-next absolute top-2/4 right-1 mr-3">
				<IconDrawerChevron />
			</div>
		</div>
	{:else}
